...
function handleKeyData(data) {
    if (data.event === 'bindings') {
        ...
        if (data.trail_width !== undefined) {
            trailWidth = data.trail_width;
        }
...
function handleKeyPress(i) {
    ...
    if (showParticles) {
        createParticles(i * colWidth + colWidth / 2 + (keyOffsetsX[i] || 0), height, colors[i], rgbEnabledKeys[i]);
    }
...
function animate(currentTime) {
    ...
    for (let i = activeBlocks.length - 1; i >= 0; i--) {
        ...
        if (showTrails) {
            const xOffset = keyOffsetsX[b.key] || 0;
            const tWidth = trailWidth || (colWidth - 10);
            const xPos = b.key * colWidth + (colWidth - tWidth)/2 + xOffset;
